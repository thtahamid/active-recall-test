...
const Card = ({ children, style = {}, depth = "md" }) => (
  <div style={{ background: c.surface, borderRadius: 18, padding: "28px 30px", boxShadow: shadows[depth], ...style }}>
    {children}
  </div>
);

const InsetPanel = ({ children, style = {} }) => (
...
const Btn = ({ children, onClick, color = c.indigo, style = {}, disabled = false }) => (
...
const H1 = ({ children }) => <h1 style={{ fontSize: 32, fontWeight: 900, margin: "10px 0 6px", color: c.text, letterSpacing: -0.5 }}>{children}</h1>;
const H2 = ({ children }) => <h2 style={{ fontSize: 22, fontWeight: 800, margin: "8px 0 6px", color: c.text }}>{children}</h2>;
const H3 = ({ children }) => <h3 style={{ fontSize: 16, fontWeight: 800, margin: "0 0 4px", color: c.text }}>{children}</h3>;
const Sub = ({ children }) => <p style={{ color: c.muted, fontSize: 14, lineHeight: 1.65, margin: "0 0 4px" }}>{children}</p>;
const ChartSub = ({ children }) => <p style={{ color: c.muted, fontSize: 13, lineHeight: 1.6, margin: "0 0 20px" }}>{children}</p>;

const page = {
  minHeight: "100vh",
  background: `radial-gradient(ellipse at 60% 0%, #ddd9ff 0%, ${c.bg} 55%)`,
  display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
  padding: "48px 20px", fontFamily: "'Inter', system-ui, sans-serif", color: c.text,
};

/* ── Word Choice Tile ── */
const WordTile = ({ word, lang, state, onClick }) => {
  // state: "idle" | "selected" | "correct" | "miss" | "false-pos"
  const styles = {
  ...
  const icons = { idle: null, selected: "●", correct: "✓", miss: "○", "false-pos": "✗" };
...
export default function App() {
  ...
  const submit = () => {
    ...
    const matched = WORDS.map(w => ({ ...w, recalled: selected.has(w.word) }));
  ...
  if (phase === "distract") return (
    <div style={page}>
      <div style={{ maxWidth: 460, width: "100%", textAlign: "center" }}>
        ...
        <Btn onClick={() => setPhase("recall")} color={c.teal} style={{ marginTop: 28, width: "100%", padding: "14px 0" }}>Skip to Recall →</Btn>
      </div>
    </div>
  );
        ...
        <Card depth="xl" style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20, flexWrap: "wrap", gap: 12 }}>
            <div style={{ fontSize: 13, color: c.muted }}>
              <strong style={{ color: c.indigo }}>{selected.size}</strong> word{selected.size !== 1 ? "s" : ""} selected
            </div>
      ...
      <div style={{ ...page, justifyContent: "flex-start", paddingTop: 48 }}>
        <div style={{ maxWidth: 880, width: "100%" }}>
            ...
            <InsetPanel style={{ padding: "20px 8px 8px" }}>
              <ResponsiveContainer width="100%" height={230}>
                <AreaChart data={serialData} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 110]} tickFormatter={v => `${v}%`} tick={{ fontSize: 11, fill: c.muted }} />
                  <Tooltip formatter={(v, _, pl) => [`${v}%`, pl.payload.word]} labelFormatter={l => `Position ${l}`} contentStyle={{ borderRadius: 10, border: `1px solid ${c.border}`, boxShadow: shadows.md }} />
            ...
            <InsetPanel style={{ padding: "20px 8px 8px" }}>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={encodingData} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
                  ...
                  <Tooltip formatter={v => [`${v}%`, "Recall Rate"]} contentStyle={{ borderRadius: 10, border: `1px solid ${c.border}`, boxShadow: shadows.md }} />
            ...
            <InsetPanel style={{ padding: "20px 8px 8px" }}>
              <ResponsiveContainer width="100%" height={230}>
                <LineChart data={forgetting} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 110]} tickFormatter={v => `${v}%`} tick={{ fontSize: 11, fill: c.muted }} />
                  <Tooltip formatter={(v, n) => [`${v}%`, n === "retention" ? "Ebbinghaus" : "Your Score"]} contentStyle={{ borderRadius: 10, border: `1px solid ${c.border}`, boxShadow: shadows.md }} />
                  <Legend formatter={v => v === "retention" ? "Ebbinghaus Curve" : "Your Score"} wrapperStyle={{ fontSize: 12 }} />
          ...
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <Btn color={c.teal} onClick={() => { setPhase("intro"); setSelected(new Set()); setSubmitted(false); setScores(null); }} style={{ padding: "14px 36px", fontSize: 15 }}>
              ↺ Run Again
            </Btn>
          </div>
        </div>
